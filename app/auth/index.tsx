...
export default function LoginScreen() {
  ...
  const onGoogle = async () => {
    ...
    try {
      ...
      setErr("No se pudo continuar con Google.");
    }
  };
  ...
  const onLogin = async () => {
    ...
    try {
      setLoading(true);
      await signInEmail(e, p);
      router.replace("/(drawer)/(tabs)");
    } catch (ex: any) {
      const msg = ex?.message?.toString?.() ?? "Error al iniciar sesión.";
      setErr(
  ...
  const onForgot = async () => {
    ...
    const e = email.trim();
    if (!e) {
      setErr("Escribe tu email para enviarte el enlace.");
      return;
    }
    try {
          ...
          <View style={styles.form}>
            {err ? (
              <View
                style={[
                  styles.msg,
                  { backgroundColor: "#e74c3c15", borderColor: "#e74c3c" },
                ]}
              >
            ...
            {info ? (
              <View
                style={[
                  styles.msg,
                  { backgroundColor: "#2ecc7115", borderColor: "#2ecc71" },
                ]}
              >
